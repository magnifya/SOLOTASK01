...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_export = sub.add_parser(
        "export", help="export a key as a passphrase-encrypted bundle"
    )
    ...
    p_import = sub.add_parser(
        "import", help="import a passphrase-encrypted key bundle"
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "export":
        ...
        if record is None:
            if not _identifiers_ok(args.tenant_id, args.key_id):
                ...
                return 1
            return _fail("key not found", 4)
        blob = encrypt_bundle(args.passphrase, build_export_payload(record))
        if not _attempt(
            store, args.tenant_id, args.key_id,
            audit_mod.ACTION_EXPORT, audit_mod.OUTCOME_SUCCESS,
        ):
            return 1
        _print({"format": EXPORT_FORMAT, "bundle": blob})
        return 0
    ...
    if args.command == "import":
        if not args.tenant_id or not args.passphrase or not args.bundle:
            ...
            return _fail("field %s must be a non-empty string" % field, 2)
        try:
            data = validate_import_payload(
                decrypt_bundle(args.passphrase, args.bundle)
            )
        ...
        key_id = data["key_id"]
        try:
            status, record = store.import_key(args.tenant_id, data)
        except LedgerError as exc:
            return _ledger_fail(exc)
        if status == "conflict":
            if not _attempt(
                store, args.tenant_id, key_id,
                audit_mod.ACTION_IMPORT, audit_mod.OUTCOME_REJECTED,
            ):
                return 1
            return _fail("key already exists", 2)
        if status == "foreign":
            if not _attempt(
                store, args.tenant_id, key_id,
                audit_mod.ACTION_IMPORT, audit_mod.OUTCOME_REJECTED,

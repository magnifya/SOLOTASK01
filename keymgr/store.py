...
class KeyStore:
    ...
    def crypto_material(
        ...
        With a finite ``lock_timeout`` both key locks share one deadline and a
        wait beyond it raises :class:`LockTimeout` (the idempotent encrypt
        path maps that to a 503 timed_out terminal); ``None`` blocks, which is
        the historical behavior used by decrypt and the CLI.
        """
        if not is_valid_key_id(key_id):
            return self.CRYPTO_NOT_FOUND, None, None, None
        path = self._path_for(key_id)
        if lock_timeout is None:
            locks_cm = self.key_locks(key_id)
        else:
            locks_cm = self._key_locks_deadline(
                key_id, time.monotonic() + lock_timeout
            )
        with locks_cm:

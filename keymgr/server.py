...
def make_handler(
    ...
    class KeyHandler(BaseHTTPRequestHandler):
        ...
        def _sign_key(self, key_id: str, parts, operator: str) -> None:
            ...
            fixed 503 text. When the version's provider declares the
            optional ``sign`` operation the signature is computed natively
            inside the KMS/HSM (no export, no private key in this process)
            and verified against the version's public key before release;
            otherwise the material is exported and signed in memory. The
            message and signature never enter the audit ledger; a private
            key, handle or wrapped material never enters a response.
            """

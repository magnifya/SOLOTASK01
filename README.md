...
- **本地提供者**仍把材料保存在数据目录，但只用数据加密密钥（`local.dek`，
  ...
  服务**不会**在启动时扫描或改写旧记录：普通读取既不加载提供者也不触碰
  文件。旧记录仅当**本地提供者为当前活动提供者**、且其材料在轮换 / 单 key
  导出 / 租户备份中**首次被需要**时，才在 per-key 锁内被惰性、原子地校验、
  包装并登记为本地对象，重写文件后旧明文字段消失；校验或写入失败保留原文件。
  活动提供者不是本地提供者时，旧记录的轮换 / 导出 / 导入一律 `503`，绝不回退。
...
- 私钥不经由密钥文件明文保存：每个版本只持久化提供者三元组
  ...
  提供者层之前的裸 `private_material` 旧记录不会在启动时被改写；仅在本地
  提供者活动且材料首次被需要（轮换 / 导出 / 备份）时惰性、原子地包装接管，
  失败保留原文件。

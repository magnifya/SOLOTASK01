...
- **主备故障转移（`KEYMGR_PROVIDER_CHAIN`）**：链配置后，首次健康激活
  选链中首个健康项。普通 provider 调用自**首次等待/探活起共用一个不可
  重置的 5 秒总预算**（不另开、不重置）；每次 `health()` 单独再限 **1
  秒**（取 1 秒与预算余量的较小者），返回 `False`、非 `bool`、抛异常或
  超时各计一次失败，迟到结果作废。**每次普通调用都先探活活动实例**：探
  活恰好返回 `True` 即把连续失败数清零并继续；否则把活动项的连续失败数
  加一（持久化、最多 3）并固定 `503`（CLI `1`），错误体固定
  `{"error":"key management provider is unavailable"}`，业务、持久化、
  审计零副作用（不铸句柄、不写审计、不外泄后端文本）。前两次失败**不切
  换**；第三次初探失败后，在现有 5 秒跨进程意图/排水门内**先用剩余预算
  再验一次活动实例**、再按**链序**逐个探活备项，仍逐次限 1 秒、共用同
  一预算：活动项恢复即清零并继续本次调用；否则只切一次到**首个健康备
  项**（排除当前活动 id）——仅一次提交 `switching`→`ready`——并令
  `generation` 加 1，在新代继续本次调用。**预算耗尽或无健康备项则固定
  `503`、旧代不变、计数封顶 3**。在途旧调用由其进入时捕获的实例完成，
  切换提交后到达的调用只用新代，并发的第三次失败仅一次建厂/提交（其余
  等待者直接采用已提交的代；失败计数本身由独立的
  `provider-health.lock` 短排他锁跨进程串行化，前两次失败不排水、不阻
  断在途调用）。主项恢复健康后**不自动回切**；只有 reconnect 按链序重
  选首个健康项。status/reconnect/switchover 不计数；毒文件不改写。
  pending 幂等操作仍绑定原 `provider_id`，绝不在备项重放；重连回同 id
  提供者后，同 `Idempotency-Key` 的请求在同一
...
- **持久化故障门限 `provider-health.json`(0600)**：记录活动项的连续失败
  ...
  按上述规则收敛；探活 `True` 清零，`False`/非 bool/异常/单次超时（普通
  调用路径上每次探活限 1 秒且共用该次 5 秒总预算，迟到结果作废）加一，
  并发失败的计数由 `provider-health.lock` 短排他锁串行（前两次失败不排
  水、不阻断在途
  调用），只有第三次失败才触发那一次切换。

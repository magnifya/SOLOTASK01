...
- 缺少必填字段、algorithm 不支持、version 非正整数、导入包口令/格式错误、
  `key_id` 非法（非 RFC 4122 UUID4：版本位须为 4、变体位须为 10xx）、
  重复 `X-Tenant-Id` 头、重复 `tenant_id` 查询参数、或
...
- 策略文档同样走 outbox 事务：`PUT` 先把策略文件（数据目录
  ...
  两步，崩溃后下次启动据墓碑幂等收尾（原文件仍在则回滚删除，否则补记并
  清理）；账本写失败时 `PUT` 恢复旧文档、`DELETE` 恢复原文件并清除墓碑，
  绝不留半成品。策略写在 per-tenant 锁（进程内锁 +
  `policies/<sha256(tenant_id)>.lock` 上的 `fcntl` 排他锁）保护下串行化。
- 租户级备份在持有该租户全部 key 锁（按 key_id 排序）与策略租户锁期间
  读取，并同时持有跨进程恢复锁，因此每次备份都是同一提交视图：版本、
  `current`、吊销字段与 policy 不混合；并发的轮换、吊销、策略更新与恢复
  要么先于快照完成，要么等待快照结束。
- 租户级恢复是跨多个 key 文件与至多一个策略文件的**多文件 outbox 事务**：
  ...
  都幂等，不重不漏。整个恢复在跨进程恢复锁（`restores/restore.lock` 上的
  `fcntl` 排他锁）保护下执行，多个进程同时恢复同一批数据时串行化：后到者
  看到先到者已提交的文件或批次标记，同租户冲突 `409`、跨租户占用 `404`。
  空包恢复（`keys: []`、`policy: null`）不建 key/策略文件，但会持久化一条
  批次幂等标记（`restores/<sha256(批次)>.json`，内含待提交事件）再补记
  一条成功事件：同一批次至多一次成功提交，重复恢复只冲突、不重复成功
  `import` 事件；崩溃后下次启动据标记幂等补记账本。
...
- `key_id` 为 RFC 4122 UUID4（版本位 4、变体位 10xx 的规范小写形式），
  读取时严格校验以杜绝路径穿越；非法值按参数错误返回 `400` 并指出
  `key_id`（CLI 退出 `2`），未知 key、未知版本与跨租户访问（含合法但
  未知的 UUID4）一律 `404`，不泄露密钥是否存在。
...
- 审计游标是 HMAC 签名的不透明令牌，密钥存于数据目录 `audit.secret`
  （权限 `0600`），首次创建在与账本追加相同的跨进程锁下串行化并原子落盘，
  多进程并发初始化不会冲突也不会读到空文件；游标绑定租户、筛选、`limit`
  与该可见结果集的快照指纹；篡改、改租户/筛选或可见事件集变化都会令游标
  失效（`400` / 退出 `2`），其他租户的活动不影响本租户游标，其他进程
  追加时读取在跨进程锁下取一致快照。事件按 `timestamp`、`event_id` 升序，
